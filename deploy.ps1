...
try {
    ...
    # Use zip deployment approach that works reliably
    Write-Host "Preparing zip deployment for App Service..." -ForegroundColor Yellow
    ...
    $filesToDeploy = @("server.js", "package.json", "web.config")
    ...
    # Create the zip file from the clean deployment folder
    ...
    # Deploy to App Service using az webapp deploy
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Warning "Zip deployment may have had issues, but continuing..."
        Write-Host "Deploy output: $deployResult" -ForegroundColor Yellow
    }

    # Clean up zip file if it exists
    if (Test-Path "mock-servicenow.zip") {
